/**
 * api-cache.js - Simple API caching with stale-while-revalidate
 */
...
(function() {
    ...
    window.addEventListener('beforeunload', function() { sessionStorage.setItem('_scrollY', window.scrollY); });
    window.addEventListener('load', function() {
        var nav = performance.getEntriesByType('navigation')[0];
        var savedY = parseInt(sessionStorage.getItem('_scrollY') || '0', 10);
        if (nav && nav.type === 'reload' && savedY > 0) {
            setTimeout(function() { window.scrollTo({ top: savedY, behavior: 'smooth' }); }, 120);
        } else {
...
(function initGlobalSearch() {
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (!input || window.__searchInitialized__) return;
...
const CACHE_TTL_SHORT = 30 * 1000, CACHE_TTL_LONG = 5 * 60 * 1000, CACHE_TTL = CACHE_TTL_LONG;
const CACHE_TTL_MAP = {'featured_products': CACHE_TTL_SHORT, 'all_products': CACHE_TTL_SHORT, 'reviews': CACHE_TTL_LONG, 'faqs': CACHE_TTL_LONG, 'banners': CACHE_TTL_LONG, 'terms': CACHE_TTL_LONG, 'privacy': CACHE_TTL_LONG, 'about_hero_subtitle': CACHE_TTL_LONG, 'about_who_label': CACHE_TTL_LONG, 'about_section_title': CACHE_TTL_LONG, 'about_story_p1': CACHE_TTL_LONG, 'about_story_p2': CACHE_TTL_LONG, 'about_story_p3': CACHE_TTL_LONG, 'about_cta_title': CACHE_TTL_LONG, 'about_cta_subtitle': CACHE_TTL_LONG, 'logo_wide': CACHE_TTL_LONG, 'logo_square': CACHE_TTL_LONG, 'top_bar_text': CACHE_TTL_LONG, 'hero_title': CACHE_TTL_LONG, 'hero_subtitle': CACHE_TTL_LONG};

function cacheGet(key) {
    try { const item = localStorage.getItem('ac_' + key); if (!item) return null; const parsed = JSON.parse(item); const ttl = CACHE_TTL_MAP[key] ?? CACHE_TTL; return { data: parsed.data, stale: Date.now() - parsed.ts > ttl }; } catch { return null; }
}

function cacheSet(key, data) { try { localStorage.setItem('ac_' + key, JSON.stringify({ data, ts: Date.now() })); } catch {} }
function cacheInvalidate(key) { try { localStorage.removeItem('ac_' + key); } catch {} }
function cacheInvalidateAll() { try { Object.keys(localStorage).filter(k => k.startsWith('ac_')).forEach(k => localStorage.removeItem(k)); } catch {} }
...
function fetchCached(url, cacheKey, onData) {
    ...
    fetch(API_BASE + url).then(r => r.json()).then(json => {
        if (json.success !== false) {
            const fresh = json.data !== undefined ? json.data : json;
            const freshStr = JSON.stringify(fresh);
            cacheSet(cacheKey, fresh);
            if (freshStr !== cachedStr) onData(fresh, false);
        }
    }).catch(() => {});
}
...
(function loadSiteLogos() {
    fetchCached('/api/content/logo_wide', 'logo_wide', function(data) {
        ...
        document.querySelectorAll('img.logo-wide, img#site-logo-wide').forEach(function(el) { if (el.src !== data.value) el.src = data.value; });
    });
    fetchCached('/api/content/logo_square', 'logo_square', function(data) {
        if (!data || !data.value || !data.value.startsWith('http')) return;
        document.querySelectorAll('img#site-logo-square').forEach(function(el) { if (el.src !== data.value) el.src = data.value; });
    });
